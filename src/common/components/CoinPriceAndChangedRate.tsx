...
const CoinPriceAndChangedRate: React.FC<ICoinPriceAndChangedRateProps> = ({
  ...
  return (
    ...
    alignItems: 'center', marginTop: '4px'}}>
          <div style={{fontSize: '14px', color: 'grey' }}>1.0000000 {symbol.toLocaleUpperCase()}</div>
          
          <div style={{marginLeft: '28px',fontSize: '14px', color: changedRate24H != null ? changedRate24H >= 0 ? changedRate24H == 0 ? 'black' : 'red': 'blue':'black'}}>{changedRate24H != null ? formatNumber(changedRate24H):'-'}%</div>
        </div>
      </div>
      <div style={{display: 'flex',flexDirection: 'row', justifyContent: 'flex-end', paddingLeft: '100px'}}>
        <div >
          ...
          <div style={{fontSize: '15px'}}>{currency === CurrencyEnum.KRW ? '₩' : '$'}{marketCap !== undefined && formatNumber(marketCap)}</div>
        </div>
        <div style={{marginLeft: '40px'}}>
          <div style={{fontSize: '15px',     marginBottom: '6px'}}>24시간 거래 대금</div>
          <div style={{fontSize: '15px'}}>{currency === CurrencyEnum.KRW ? '₩' : '$'}{totalVolume24H !== undefined && formatNumber(totalVolume24H)}</div>
        </div>
      </div>
    </div>
  )
